...
const createWindow = () => {
  ...
  menu.items[0].submenu.append(new MenuItem({ //menu.items获取是的主菜单一级菜单的菜单数组，menu.items[0]在这里就是第1个File菜单对象，在其子菜单submenu中添加新的子菜单
    label: "新建",
    click(){
      mainWindow.webContents.send('file', 'new'); //点击后向主页渲染进程发送“新建文件”的命令
    },
  ...
  menu.items[0].submenu.append(new MenuItem({
    label: "打开",
    click(){
      mainWindow.webContents.send('file', 'open'); //点击后向主页渲染进程发送“打开文件”的命令
    },
  ...
  menu.items[0].submenu.append(new MenuItem({
    label: "Save",
    click(){
      mainWindow.webContents.send('file', 'save'); //点击后向主页渲染进程发送“保存文件”的命令
    },
  ...
  mainWindow.on('close', (e) => {
      ...
      // e.preventDefault();
      mainWindow.webContents.send('file', 'exiting');
